...
class ImageNetDataset(Dataset):
    def __init__(self, data_root, image_size, n_class=1000, partition="train", device="cpu"):
...
def get_args_parser():
    ...
    parser.add_argument("--imagenet_path", default="/mnt/nasv2/MILab/zhulei/nasv1_dmp/datas/imagenet", type=str, help="path of llama model")
    ...
    parser.add_argument("--rate_s_local", type=float, default=1, help="Decoding Loss")
    parser.add_argument("--rate_s_global", type=float, default=1, help="Decoding Loss")
    parser.add_argument("--rate_d", type=float, default=1, help="VGG Loss")
    ...
    parser.add_argument("--local_embedding_path", default="clip_codebook_1st_2nd_filtering.pth", type=str, help="path of llama model")
    parser.add_argument("--use_global_branch", type=int, default=0, help="Using Global Branch")
    parser.add_argument("--global_embedding_path", default="clip_codebook_1st_2nd_filtering.pth", type=str, help="path of llama model")
...
def main(args):
    ...
    #model_fid = InceptionV3([block_idx], normalize_input=False).to(device)
    model_fid = InceptionV3([block_idx]).to(device)
    model_fid.eval()
    start_idx = 0
    pred_arr_source = np.empty((len(dataset_val.image_ids), 2048))
    pred_arr_target = np.empty((len(dataset_val.image_ids), 2048))

    inception_model = inception_v3(pretrained=True, transform_input=False).to(device)
    inception_model.eval()
    pred_arr_inc = np.empty((len(dataset_val.image_ids), 1000))
    pred_arr_class = np.empty((len(dataset_val.image_ids)))
    ...
    source_save_dir = os.path.join(args.output_dir, "source")
    ...
    for data_iter_step, [images, clip_image, label_cls] in enumerate(
        ...
        for b in range(0, save_x.shape[0]):
            plt.imsave(os.path.join(source_save_dir, "%s.png"%(count)), np.uint8(save_x[b].detach().cpu().numpy().transpose(1, 2, 0) * 255))
    ...
    ###INC
    split_scores = []
    N = pred_arr_inc.shape[0]
    splits = 10
    for k in range(splits):
        part = pred_arr_inc[k * (N // splits): (k+1) * (N // splits), :]
    ...
    #plt.imsave(os.path.join(args.output_dir, "token_freq.png"), np.array(token_freq.view(200, 160).cpu().data))
    ...
    with open(os.path.join(args.output_dir, "recons.csv"), 'a') as f:
        f.write("FID, IS, LPIPS PSNR, SSIM, Effective_Tokens \n")
        f.write("%.4f, %.4f, %.4f, %.4f, %.4f, %d \n"%(fid_value, is_value, lpips_total.item()/num_images, psnr_total.item()/num_images, ssim_total.item()/num_images, efficient_token))
