...
class MiniImageNetDataset(Dataset):
    def __init__(self, data_root, image_size, time, model_path, vision_tokenizer, clip_texts, clip_codebook, text_token_embedding, way=2, shot=1, device="cpu", induction=1):
    ...
    def __getitem__(self, index):
        ...
        for i in range(0, len(image_ids)):
            ...
            image_clip = self.preprocess(image).unsqueeze(0)
            ...
            select_images_clip.append(image_clip)


        images = torch.from_numpy(np.concatenate(select_images, 0))
        images_clip = torch.from_numpy(np.concatenate(select_images_clip, 0))
        images = images.to(self.device)
        images_clip = images_clip.to(self.device)
        ##Transfer image to text with V2L Tokenizer (Global Branch)
        with torch.no_grad():
            _, d_idx_global_local = self.vision_tokenizer(images, images_clip, 0, step=0, is_val=True, k=args.token_num)
        d_idx = d_idx_global_local[:, :args.token_num].contiguous()
        ...
        return [prompt_tokens, prompt_features, target_texts, out_mask, max_len, image_ids]
...
def main(args):
    ...
    clip_codebook = torch.load("codebooks/global_codebook_embedding.pth").to(args.device)
    ...
    if os.path.exists(os.path.join(args.output_dir, "%d-k_%d-way_%d-shot_%d-repeat_%d-induction.npy"%(args.token_num, args.way, args.shot, args.repeat, args.induction))):
        dump = np.load(os.path.join(args.output_dir, "%d-k_%d-way_%d-shot_%d-repeat_%d-induction.npy"%(args.token_num, args.way, args.shot, args.repeat, args.induction)), allow_pickle=True).item()
        acc = dump["acc"]
        time = dump["time"]
        #dataset_train = dataset_train[time:]
        dataset = MiniImageNetDataset(
            data_root=args.mini_imagenet_path, image_size=args.image_size, time=time, clip_texts=clip_texts, clip_codebook=clip_codebook, model_path=args.llama_model_path, vision_tokenizer=model, text_token_embedding=generator.model.tok_embeddings, way=args.way, shot=args.shot, device=device, induction=args.induction
        )
    else:
        dataset = MiniImageNetDataset(
            data_root=args.mini_imagenet_path, image_size=args.image_size, time=0, clip_texts=clip_texts, clip_codebook=clip_codebook, model_path=args.llama_model_path, vision_tokenizer=model, text_token_embedding=generator.model.tok_embeddings, way=args.way, shot=args.shot, device=device, induction=args.induction
        )
    ...
    for data_iter_step, [prompt_tokens, prompt_features, target_texts, out_mask, max_lens, image_ids] in enumerate(
        ...
        if time % 2000 == 0: #Save Acc and Times for Resuming
            ...
            save_np["acc"] = acc
            save_np["time"] = time
            np.save(os.path.join(args.output_dir, "%d-k_%d-way_%d-shot_%d-repeat_%d-induction.npy"%(args.token_num, args.way, args.shot, args.repeat, args.induction)), save_np)

    print("Accuracy: ", acc / time)
    with open(os.path.join(args.output_dir, "few_shot.csv"), 'a') as f:
        f.write("%d, %d, %d, %.4f \n"%(args.way, args.shot, time, acc / time))
